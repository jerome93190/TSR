...
/* ============================================================
   LIVE BRIDGE VIEW
   ============================================================ */
const LIVE_BRIDGE_PY = `# Aperçu — extrait du fichier complet (~200 lignes) sur GitHub.
import asyncio, ctypes, json, mmap, struct, time, websockets
from ctypes import c_double, c_long, c_char, Structure

TELEMETRY_NAME = r"$rFactor2SMMP_Telemetry$"  # ou $lmuSMMP_Telemetry$ pour Racelab
...
class rF2VehicleTelemetryHead(Structure):
    # Subset de la struct C++ rF2VehicleTelemetry (plugin v3.7+).
    # ctypes calcule les offsets exacts de chaque champ declare.
    _pack_ = 4
    _fields_ = [
        ("mID", c_long), ("mDeltaTime", c_double), ("mElapsedTime", c_double),
        ("mLapNumber", c_long), ("mLapStartET", c_double),
        ("mVehicleName", c_char * 64), ("mTrackName", c_char * 64),
        ("mPos", rF2Vec3), ("mLocalVel", rF2Vec3), ("mLocalAccel", rF2Vec3),
        ("mOri", rF2Vec3 * 3), ("mLocalRot", rF2Vec3), ("mLocalRotAccel", rF2Vec3),
        ("mGear", c_long), ("mEngineRPM", c_double), ("mEngineWaterTemp", c_double),
        ...
        # ... + ride heights, downforce, mFuel, mEngineMaxRPM
    ]

def read_snapshot(mm):
    mm.seek(0)
    update_begin, update_end, num_vehicles = struct.unpack("<IIi", mm.read(12))
    if num_vehicles < 1 or update_begin != update_end:
        return None
    mm.seek(12)
    veh = rF2VehicleTelemetryHead.from_buffer_copy(
        mm.read(ctypes.sizeof(rF2VehicleTelemetryHead)))
    speed = (veh.mLocalVel.x**2 + veh.mLocalVel.y**2 + veh.mLocalVel.z**2)**0.5 * 3.6
    return {"speed_kmh": speed, "rpm": veh.mEngineRPM, "gear": veh.mGear,
            "fuel_l": veh.mFuel, "throttle": veh.mFilteredThrottle,
            "brake": veh.mFilteredBrake, "steering": veh.mFilteredSteering}

# WebSocket loop diffuse 30 Hz à tout client connecté
# -> code complet : https://github.com/jerome93190/tsr/blob/claude/lemans-web-app-ideas-iCtbP/bridge/tsr_live_bridge.py
`;
...
function renderLive() {
  ...
  root.innerHTML = `
    ...
    ${tab === 'instructions' ? `
      <div class="chart-box">
        <h4>Mise en route — option rapide (.exe)</h4>
        <ol style="line-height:1.8;color:var(--text);font-size:14px;">
          ...
          <li>Télécharger le bridge prêt à l'emploi (compilé automatiquement par GitHub Actions) :
            <div style="margin:10px 0;">
              <a class="btn primary" href="https://github.com/jerome93190/tsr/releases/download/bridge-latest/tsr_live_bridge.exe" target="_blank" rel="noopener">⬇ tsr_live_bridge.exe (Windows)</a>
              <a class="btn ghost" href="https://github.com/jerome93190/tsr/releases/tag/bridge-latest" target="_blank" rel="noopener" style="margin-left:8px;">Voir la release</a>
            </div>
            <p class="muted" style="font-size:12px;margin:6px 0 0;">~10 MB · zéro dépendance · double-clic pour lancer · Windows 10/11 64 bits.</p>
          </li>
          <li>Double-cliquer sur <code>tsr_live_bridge.exe</code>. Une console s'ouvre et affiche :
            <div class="code-block">TSR Live Bridge\nListening on ws://localhost:8765</div>
          </li>
        ...
        <p class="muted" style="margin-top:16px;">Le bridge écoute sur <code>0.0.0.0:8765</code> donc une tablette/téléphone sur le même Wi-Fi peut s'y connecter en second écran (autoriser le port dans Windows Defender Firewall).</p>
        <p class="muted" style="margin-top:12px;font-size:13px;">Préférer Python ? Voir l'onglet "Bridge Python" → script source à lancer avec <code>python tsr_live_bridge.py</code>.</p>
      </div>
    ` : tab === 'bridge' ? `
      <div class="chart-box">
        <h4>tsr_live_bridge.py — source Python</h4>
        <p class="muted" style="margin-bottom:12px;">Pour les puristes : lance directement avec <code>python tsr_live_bridge.py</code> (nécessite <code>pip install websockets</code>).</p>
        <div style="margin-bottom:16px;">
          <a class="btn primary" href="https://raw.githubusercontent.com/jerome93190/tsr/claude/lemans-web-app-ideas-iCtbP/bridge/tsr_live_bridge.py" target="_blank" rel="noopener" download="tsr_live_bridge.py">⬇ tsr_live_bridge.py</a>
          <a class="btn ghost" href="https://github.com/jerome93190/tsr/blob/claude/lemans-web-app-ideas-iCtbP/bridge/tsr_live_bridge.py" target="_blank" rel="noopener" style="margin-left:8px;">Voir la source GitHub</a>
        </div>
        <p class="muted" style="font-size:13px;margin-bottom:8px;">Aperçu — la classe <code>rF2VehicleTelemetryHead</code> reproduit la struct C++ du plugin v3.7+ avec ctypes, ce qui garantit le bon offset de chaque champ :</p>
